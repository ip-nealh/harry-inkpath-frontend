...
export const Form: React.FC<Props> = ({ token }) => {
    ...
    const generateQuery = () => {
        ...
        switch (mode) {
            case 'convert':
                return `https://harry-inkpath-api.vercel.app/convert${string}&amount=${value}`;
            case 'historical':
                return `https://harry-inkpath-api.vercel.app/historical${string}&date=${moment(historicalDate).format('YYYY-MM-DD')}`;
            case 'range':
                return `https://harry-inkpath-api.vercel.app/range${string}&start=${moment(startDate).format('YYYY-MM-DD')}&end=${moment(endDate).format('YYYY-MM-DD')}`
            default:
                return ``;
        }
